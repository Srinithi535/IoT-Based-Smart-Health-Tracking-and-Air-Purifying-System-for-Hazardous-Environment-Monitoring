...
void setup() {
  ...
  if (readRegister(REG_PART_ID) != 0x11) {
    ...
    lcd.clear();
    lcd.setCursor(0, 0); lcd.print("MAX30100 Error!");
    while (1);
  }
  ...
  if (millis() - lastPrint >= 1000) {
    ...
    if (!fingerOn) {
      ...
      if (beatDetector.avgBPM > 0) {
        Serial.print(beatDetector.avgBPM, 1);
        Serial.println(" BPM");
      } else {
